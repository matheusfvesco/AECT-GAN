...
if __name__ == "__main__":
    ...
    datasetClass, augmentationClass, dataTestClass, collateClass = get_dataset(
        opt.dataset_class
    )
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for epoch_i, data in enumerate(dataloader):
            ...
            if total_steps % opt.print_freq == 0:
                ...
                lr = gan_model.optimizers[0].param_groups[0]["lr"]
                loss_str = ", ".join(
                    [
                        f"{key}:{(value.item() if hasattr(value, 'item') else value):.4f}"
                        for key, value in losses.items()
                    ]
                )
                print(
                    f"[{current_time}] Epoch {epoch} Iter {epoch_i}/{dataset_size} "
                    f"Step {total_steps} Time {t1 - t0:.3f}s LR {lr:.6f} | {loss_str}"
                )
            ...
            total_loss = visualizer.add_total_scalar(
                "Total loss", loss_dict, step=total_steps
            )
            ...
            with torch.no_grad():
                (
                    ...
                    temp_D_loss_lpips,
                    temp_D_loss_mse,
                ) = gan_model.return_D_G_epochLoss()
