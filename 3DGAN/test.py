...
def evaluate(args):
    ...
    for epoch_i, data in tqdm.tqdm(enumerate(dataloader)):
        ...
        lpips = LPIPS(
            torch.from_numpy(real_CT_transpose).unsqueeze(0),
            torch.from_numpy(generate_CT_transpose).unsqueeze(0),
            percept,
            size_average=False,
        )
        # CT range 0-4096
    ...
    for key, value in avg_dict.items():
        ...
        if (key == "SSIM-TS") or (key == "SSIM-CS") or (key == "SSIM-SS"):
            ssim_last.append(np.mean(value))
        if (key == "LPIPS-TS") or (key == "LPIPS-CS") or (key == "LPIPS-SS"):
