...
def parse_args():
    ...
    parse.add_argument("--tag", type=str, default="indiana_comparison", dest="tag",
    ...
    parse.add_argument("--model_root", type=str, default=None, dest="model_root",
            ...
            for row_idx, slice_idx in enumerate(slice_indices):
                ...
                for col_idx, (variant_key, label) in enumerate(MODEL_VARIANTS):
                    ...
                    if variant_key in variant_slices:
                        ax_var.imshow(variant_slices[variant_key], cmap="gray",
                                      vmin=all_min, vmax=all_max, interpolation="nearest")
                        if row_idx == 0:
...
def create_html_visualization(samples, output_dir, checkpoint_num):
    ...
    for name, variant_samples in tqdm(grouped.items(), desc="Creating HTML per patient"):
        ...
        for slice_idx in slice_indices:
            ...
            for col_idx, (variant_key, label) in enumerate(MODEL_VARIANTS):
                if variant_key in variant_slices:
                    axes[col_idx].imshow(variant_slices[variant_key], cmap="gray",
                                         vmin=all_min, vmax=all_max, interpolation="nearest")
...
"""
        for slice_idx in slice_indices:
            slice_path = variant_slice_paths[MODEL_VARIANTS[0][0]][slice_indices.index(slice_idx)]
...
def load_checkpoint(result_dir, checkpoint_num):
    ...
    if checkpoint_path.exists():
        ...
        print(f"Loaded checkpoint: {checkpoint_path} ({len(checkpoint_data['samples'])} samples already processed)")
...
def generate_visualizations(args):
    ...
    result_dir = Path("outputs") / "supplementary" / "data" / data_path / "comparison_indiana" / f"checkpoint_{checkpoint_num}"
    ...
    pdf_path = result_dir / f"indiana_comparison_visuals_{checkpoint_num}.pdf"
