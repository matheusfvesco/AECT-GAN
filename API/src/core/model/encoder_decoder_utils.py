...
class Upsample_3DUnit(nn.Module):
    def __init__(
        ...
        if upsample_mode == "trilinear" or upsample_mode == "nearest":
            ...
            self.block = Upsample_TransposedConvBlock(
                kernel_size,
                input_channel,
                output_channel,
                norm_layer,
    ...
    def __init__(
        ...
        conv_block += [
            nn.ConvTranspose3d(
                ...
                bias=use_bias,
                stride=scale_factor,
                output_padding=int(kernel_size // 2),
            ),
            norm_layer(output_channel),
            activation,
        ]
...
class Resnet_3DBlock(nn.Module):
    def __init__(self, dim, norm_layer, activation=nn.ReLU(True), use_bias=True):
...
class Resnet_ST3DBlock(nn.Module):
    def __init__(self, dim, expand_factor, norm_layer, activation, use_bias):
        ...
        self.conv_block = self.build_conv_block(
            dim, expand_factor, norm_layer, activation, use_bias
        )

    def build_conv_block(
        self, dim, expand_factor, norm_layer, activation=nn.ReLU(True), use_bias=True
    ):
        conv_block = []
        expand_dim = int(expand_factor * dim)
        conv_block += [
            nn.Conv3d(dim, expand_dim, kernel_size=(1, 1, 1), padding=0, bias=use_bias),
        ...
        conv_block += [
            nn.Conv3d(expand_dim, dim, kernel_size=(1, 1, 1), padding=0, bias=use_bias),
            norm_layer(dim),
        ]
        return nn.Sequential(*conv_block)
...
class _DenseLayer2D(nn.Sequential):
    def __init__(
        self, num_input_features, growth_rate, bn_size, norm_layer, activation, use_bias
    ):
...
class DenseBlock2D_Transition(nn.Module):
    def __init__(
        ...
        self.conv_block = _DenseBlock2D_Transition(
            num_input_features, num_output_features, norm_layer, activation, use_bias
        )
...
class Dimension_UpsampleCutBlock(nn.Module):
    def __init__(
        ...
        compress_block = [
            nn.Conv2d(
                input_channel, output_channel, kernel_size=1, padding=0, bias=use_bias
            ),
    ...
    def forward(self, input):
        ...
        return self.conv_block(
            ...
            .unsqueeze(2)
            .expand(N, self.output_channel, H, H, W)
        )
...
class Transposed_And_Add(nn.Module):
    ...
    def forward(self, *input):
        return input[0].permute(*self.permuteView1) + input[1].permute(
            *self.permuteView2
        )
